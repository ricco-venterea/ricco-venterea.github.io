...
I have many interests at this point, but my current focus is in the cosmic microwave background and studying temperature fluctuations across the sky. See below for high level overviews of my research projects and where my research has taken me! (I only describe projects where some form of publication was made).

# Research In Progress

## ACTeroids
<img src="/images/actpol_logo.png" alt="ACTPOL Logo" width="120" style="float: right; margin-left: 15px;"/>
...
## Cosmic Rays
<img src="/images/hvo_logo.png" alt="HVO Logo" width="90" style="float: right; margin-left: 15px;"/>

Cosmic rays (CRs) are highly energetic particles produced from astrophysical events. The Sun also generates these particles, streaming through earth's atmosphere. The goal of my project was to increase access to solar CR particles by developing a website. Building on work from my colleague, I optimized this website to run more efficiently. I also quantified the relationship between the number of CRs measured on earth and the number of sunspots. This is possible because CRs are heavily influenced by magnetic fields (with a stronger field deflecting particles). Sunspots are generated by magnetic fields on the sun's surface, so it is therefore possible to correlate the presence of sunspots and amount of CRs detected on earth.

## Aframe
<img src="/images/ligo_logo.png" alt="LIGO Logo" width="90" style="float: right; margin-left: 15px;"/>

[Aframe](https://github.com/ML4GW/aframe) is a machine learning pipeline that aims to detect gravitational waves (GW) from compact binary coalescences data. This is the first pipeline using neural networks to run online at low latency. The ultimate goal of Aframe is to make GW astronomy more efficient without compromising sensitivity. We hope Aframe will speed up developments and discoveries in GW astronomy. I created a user interface for future users to examine the distribution of Aframe training parameters, which helps with validating the Aframe algorithms. I was also briefly involved in extending this pipeline to run over a single detector, which would increase data as the current implementation of Aframe at the time required two running interferometers.
